...
public sealed class TagDialogService
{
    ...
    public OperationResult? PromptTagUpdate(Func<string, Color?, OperationResult> tryAccept, string startingName,
        Color? startingColor)
    {
        return OpenTagDialog(tryAccept, "Edit Tag", "Save", startingName, startingColor);
    }

    private OperationResult? OpenTagDialog(Func<string, Color?, OperationResult> tryAccept, string title,
        string submitText, string? startingName, Color? startingColor)
    {
